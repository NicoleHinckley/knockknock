...
class FIRStorageService {
    ...
    func postImagesRef() -> StorageReference {
        return storage.reference().child("postImages")
    }
    ...
    func addPostImage(withImageData data: Data,  completion : @escaping (_ downloadURL : URL?, _ error : Error?) -> ()){
     
        // Create a reference to the file you want to upload
        let ref = postImagesRef().child(UUID.init().uuidString)
